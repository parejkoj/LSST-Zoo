...
if __name__ == "__main__":
    ...
    for target_tract in [0]:
        ...
        for target_patch in list(unique_patches):
            ...
            cutouts, cutout_data, (z1, z2), sci_cutouts = make_cutouts_for_patch(butler, tract_info, target_patch, tracklets[sel])
            if len(cutouts) == 0:
                continue

            for group_n, cutout_group in enumerate(group_items(zip(cutouts, cutout_data, sci_cutouts), 1*1)): #4*4
                plt.figure(1).clear()
